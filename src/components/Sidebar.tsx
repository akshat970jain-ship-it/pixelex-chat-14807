      ...
      <div className="p-6 border-b border-border">
        <div className="relative w-24 h-24 mx-auto mb-4">
          <Avatar className="w-24 h-24">
            <AvatarImage src={isGuest ? "https://api.dicebear.com/7.x/avataaars/svg?seed=guest" : profile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile?.username}`} />
            <AvatarFallback>
              {loading ? "..." : isGuest ? "GU" : profile ? getInitials(profile.full_name) : "U"}
            </AvatarFallback>
          </Avatar>
          {!isGuest && <button className="absolute bottom-0 right-0 w-8 h-8 bg-primary rounded-full flex items-center justify-center hover:bg-primary/90 transition-colors">
